"""
Agent field auto-population utilities for new requisitions.
Ensures all P2P engine agent fields are populated when a requisition is created.
"""
...
PAYMENT_METHODS_LIST = ["ACH", "ACH", "ACH", "Wire", "Check"]  # 60% ACH
...
def populate_agent_fields(req: "Requisition") -> None:
    """
    Populate all P2P engine agent fields for a requisition.
    Called automatically when creating a new requisition.
    """
    ...
    req.audit_trail = json.dumps([
        {"action": "created", "by": req.requestor_id, "at": str(today)},
        {"action": "submitted", "by": req.requestor_id, "at": str(today)}
    ])
    ...
    req.payment_method = random.choice(PAYMENT_METHODS_LIST)
    ...
    req.payment_status = "pending"  # New requisitions start as pending
    
    # Historical Fraud fields (ALL CLEAN for new requisitions)
